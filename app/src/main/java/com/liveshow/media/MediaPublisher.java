...
public class MediaPublisher {
    ...
    private VideoGather mVideoGatherer;
    private AudioGather mAudioGatherer;
    private MediaEncoder mMediaEncoder;

    private RtmpPublisher mRtmpPublisher;
    public boolean isPublish;

    private AudioGather.Params audioParams;
    private VideoGather.Params videoParams;
...
//    public static final String url = "rtmp://192.168.155.1:1935/live/test";
     ...
     */
    public void init() {
        mVideoGatherer = new VideoGather();
        mAudioGatherer = new AudioGather();
     ...
     * 发布
     */
    public void starPublish(String url) {
        if (isPublish) {
            return;
        }
        mUrl = url;
        Runnable runnable = new Runnable() {
            ...
            public void run() {
                //初始化
                int ret = mRtmpPublisher.init(mUrl, videoParams.previewWidth, videoParams.previewHeight, 1000);
                if (ret < 0) {
                    Log.e(TAG, "连接失败");
                    return;
                }
    ...
    private void setListener() {
        mVideoGatherer.setCallback(new VideoGather.Callback() {
            @Override
        ...
        mAudioGatherer.setCallback(new AudioGather.Callback() {
            @Override
